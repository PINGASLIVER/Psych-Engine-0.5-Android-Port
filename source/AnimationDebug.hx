...
class AnimationDebug extends FlxState
{
	...
	override function create()
	{
		...
		ghostAnim = new FlxUIInputText(50, 80, 70, "", 8);
		typingShit = ghostAnim;
		...
		var changeButton:FlxButton = new FlxButton(150, 80, "Change Ghost Anim", function()
		{
			changeAnim();
		});
		...
		characterTab.add(ghostAnim);
		characterTab.add(changeButton);
	...
	function changeAnim()
	{
		if (ghostBF.animOffsets.exists(typingShit.text))
			ghostBF.playAnim(typingShit.text);
		else
			trace('no anim called ' + typingShit.text);
	}
